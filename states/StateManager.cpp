	...
	: m_shared(l_shared)
{
	...
	RegisterState<State_History>(StateType::History);
	RegisterState<State_Settings>(StateType::Settings);
	RegisterState<State_Loading>(StateType::Loading);
	RegisterState<State_EndCredits>(StateType::EndCredits);
	RegisterState<State_LevelsOrContinue>(StateType::LevelsOrContinue);
...
void StateManager::HandleInput(const sf::Event& event) {
	...
	if (m_states.back().second->IsTranscendent() && m_states.size() > 1) {
		...
		m_states.back().second->HandleInput(event);
	}
}
